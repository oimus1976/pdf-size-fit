...
For `image-heavy` inputs, the integrated standard path uses a bounded stop-on-first-success strategy: full-resolution JPEG quality probes `100 -> 90 -> 75 -> 70`, filtered by any higher configured quality floor. If `min_scale < 1.0`, it then uses a small bounded scale-probe set and stops after the first validated candidate that meets the byte target. Direct API/CLI calls keep downsampling opt-in with the `min_scale=1.0` default, while the standard simple GUI now supplies the reviewed `min_scale=0.50` floor so ordinary image-heavy fitting can complete in one bounded request.
...
Route and backend evidence are hidden from the default view. Quality and scale controls remain under `詳細設定`; when automatic downsampling is actually used, the completion summary reports the selected scale and JPEG quality as quality-loss evidence. Simple mode keeps `min_quality=70`, automatically permits bounded image downsampling down to `min_scale=0.50` only after full-resolution probes fail, and keeps searchable-text rasterization off. The launcher uses `.venv\Scripts\pythonw.exe` and shows setup instructions if the expected runtime or installed project is missing. The same GUI is available through the `pdf-size-fit-gui` Python entry point.

Simple mode now makes one integrated backend request. Image-heavy processing exhausts the bounded full-resolution quality probes first and only then, when needed, continues through the reviewed bounded scale probes down to 50%. There is no second downsampling confirmation/retry. If scaling is used successfully, the simple result warns that image quality may have decreased and records the selected scale and JPEG quality. If the bounded search still cannot reach 10 MB, no output is left behind; page splitting remains a later Issue #21 phase. The original PDF and pre-existing collision names remain unchanged.
