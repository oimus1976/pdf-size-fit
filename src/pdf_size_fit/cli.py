...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diagnose an oversized PDF and propose a PoC routing class.")
    ...
    if args.as_json:
        ...
        print(f"image streams: {diagnosis.image_stream_bytes} bytes ({diagnosis.image_ratio:.1%})")
        print(f"vector/content streams: {diagnosis.vector_stream_bytes} bytes ({diagnosis.vector_ratio:.1%})")
