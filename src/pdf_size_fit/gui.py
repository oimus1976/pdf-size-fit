...
class _Application:
    def __init__(
        ...
        *,
        ...
        self.events: queue.Queue[tuple[str, Any, bool, GuiRequest]] = queue.Queue()
        self.busy = False
    ...
    def _worker(
        ...
        try:
            ...
            self.events.put(("result", result, simple, request))
        except BaseException as error:
            self.events.put(("error", error, simple, request))

    def _poll(self) -> None:
        try:
            kind, value, simple, request = self.events.get_nowait()
        except queue.Empty:
            if self.busy:
                self.root.after(100, self._poll)
            return
