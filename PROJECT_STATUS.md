...
Route and backend evidence are absent from the default view. Quality and scale controls remain behind `詳細設定`; successful automatic downsampling reports the selected scale and JPEG quality as quality-loss evidence. Simple requests retain `min_quality=70` and `allow_small_searchable_text_rasterization=False`, while using the reviewed `min_scale=0.50` floor for bounded automatic image downsampling after full-resolution probes fail. Processing remains on a worker thread and successful runs retain `フォルダーを開く`.
