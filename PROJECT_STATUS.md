...
Because this is destructive whole-page rasterization, a preflight gate runs before diagnosis rendering. It refuses encryption, signatures/certification permissions, any AcroForm, embedded/associated files, annotations, PDF/A identification, outlines/bookmarks, unsupported document-level navigation semantics, and by default any non-whitespace extractable text. It also refuses page geometry outside the currently proven boundary. After specific refusals, minimal catalog/page allowlists reject every dictionary key the route does not reconstruct, including unknown or custom semantics.
...
The bilevel gate inspects the same fixed 300-dpi grayscale render used by execution. For each luminance-33..246 midtone it uses Pillow 5x5 minimum/maximum filters to require both luminance-0..32 near-black and luminance-247..255 near-white support. It builds a mask of unsupported midtones and applies a 3x3 minimum filter; any survivor refuses as a persistent region. No percentage threshold is used. Sub-3-pixel grayscale detail at 300 dpi is below this persistence criterion and may be binarized, so this remains a provisional PoC guardrail rather than a general quality guarantee. Rendering or filtering uncertainty fails closed. Accepted output is reopened and checked for page count, MediaBox dimensions, rotation, 1-bit CCITT encoding, and target size before an exclusively created destination is retained.
