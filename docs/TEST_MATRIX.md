...
The fixed 300-dpi execution route now has synthetic regression cases for successful 1-bit CCITT Group 4 reconstruction, source immutability, destination non-overwrite, skip and route-mismatch outcomes, explicit `target-not-met`, page-count/MediaBox/rotation preservation, exact raster dimensions, accepted one-pixel-per-axis PDFium raster rounding, rejected two-pixel mismatches, and refusal of signatures/certification structures, any AcroForm, embedded files, annotations, PDF/A identification, encryption, outlines/bookmarks, unsupported crop geometry, invalid rotation, searchable text by default, material chroma, and persistent unsupported midtones. Raw page-tree coverage includes custom root/nested keys, Count and Parent mismatch, duplicate/repeated/cyclic identity, missing/invalid Type, exact flattened identity/order agreement, and accepted inherited Rotate/MediaBox; existing catalog and raw/effective leaf allowlists remain covered. The 300-dpi RGB gate retains hard black/white, refuses thin and material saturated features at channel spread >=16 without an area threshold, fails closed on inspection uncertainty, and verifies the candidate still uses PDFium grayscale rendering followed by Pillow 1-bit conversion. The one-pixel allowance applies only to computed 300-dpi RGB/grayscale raster dimensions, not PDF page geometry. Empty or whitespace-only extraction does not itself refuse. The explicit text opt-in is covered for exact-bound dual-parser agreement, independent pypdf/PDFium per-page and document bounds, metric disagreement, both parser failure paths, and reasons that omit extracted content. The persistent bilateral cases retain hard black/white and a narrow antialias ramp, refuse uniform gray, 3px/4px gray stripes, and a 3x3 gray patch, and record 1px/2px stripes and 1x1/2x2 patches as below the provisional criterion rather than generally safe. PDFium render and Pillow filter failures fail closed. No chroma or midtone area-percentage threshold remains. Sub-3-pixel grayscale detail at 300 dpi may be binarized. The text limits, RGB threshold, and persistence behavior are provisional PoC guardrails, not a general policy, color-science, or quality guarantee. These synthetic cases do not replace the private T01 visual-validation gate, and no private municipal PDF is stored in the repository.
